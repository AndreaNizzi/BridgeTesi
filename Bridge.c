...
void save_block(FlowBlock *block, int block_id, const char* mode_str, double limit_val) {
    char filename[512];
    FILE *fpf;

    // 1. Salva Input per LLM
    snprintf(filename, sizeof(filename), "%s_%s%g_llm_input_block_%03d.json", block->pcap_name, mode_str, limit_val, block_id);
    ...
    // 2. Salva y_true
    snprintf(filename, sizeof(filename), "%s_%s%g_y_true_block_%03d.json", block->pcap_name, mode_str, limit_val, block_id);
    fpf = fopen(filename, "w");
    if (fpf) {
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        fprintf(stderr, "[ERRORE]: Utilizzo: %s <file_pcap> <modalità:time|count> <valore>\n", argv[0]);
        return 1;
    }
    ...
    char *mode = argv[2];
    double limit_value = atof(argv[3]);
    ...
    if (strcmp(mode, "time") == 0) {
        ...
        fprintf(stderr, "[ERRORE]: Modalità '%s' non valida. Usa 'time' o 'count'.\n", mode);
        return 1;
    }

    // Validazione del valore
    if (limit_value <= 0) {
        fprintf(stderr, "[ERRORE]: Il valore deve essere maggiore di 0.\n");
        return 1;
    }
    ...
    reset_block(&current_block, 0.0);
    
    printf("[INFO]: Inizio raggruppamento flussi (Modo: %s, Valore: %g)\n", mode, limit_value);
    ...
    while (getline(&line, &len, fp) != -1) {
        ...
        if (current_block.flow_count > 0) {
            if (is_time_mode) {
                // Se la differenza di tempo supera il limite
                if (fs - current_block.start_timestamp >= limit_value) {
                    should_save = 1;
                }
            } else {
                // Se il numero di flussi ha raggiunto il limite
                if (current_block.flow_count >= (int)limit_value) {
                    should_save = 1;
                }
            }
        }
        ...
        if (should_save) {
            save_block(&current_block, block_counter++, mode, limit_value);
            reset_block(&current_block, fs);
        }

        // Inizializza il timestamp se il blocco è appena stato resettato
        if (current_block.flow_count == 0) {
           current_block.start_timestamp = fs;
        }
